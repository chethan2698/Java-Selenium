...
public class EmailUtility {
	...
	public static void sendMail() {
		...
		final String senderEmail = "noreply-test@stryker.com";
		final String senderPassword = "";
		final String recipientEmail = "baban.kadke@stryker.com";
	...
	public static void sendMailWithAttachment(String ToAddress,String CcAddress,String filepath) {

		try {
			Properties properties = new Properties();
			JSONParser parser = new JSONParser();
			String jsonConfig=System.getProperty("user.dir")+"/email/sendemail.json";
			final JSONObject config = (JSONObject) parser.parse(new FileReader(jsonConfig));

			Map<String, String> sendemailserverdata = (Map<String, String>) config.get("emailserverdata");
			Iterator	it = sendemailserverdata.entrySet().iterator();
			while (it.hasNext()) {
			...
			stringBuilder.append("<p>Chethan A T</p>");
			stringBuilder.append("</body>");
			stringBuilder.append("</html>");

			// Create a new Session instance
			Object data = config.get("username");
			Session session = Session.getInstance(properties, new Authenticator() {
				@Override
				protected PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication((String)config.get("username"), decrypt((String)config.get("password")));
				}
			});
	...
	public static void draftMail() {
		final String email = "chethan.at@stryker.com";
		final String password = "U21hcnRANDIyNzIj";

		// Mail properties
		Properties props = new Properties();
		props.put("mail.smtp.auth", "true");
		props.put("mail.smtp.starttls.enable", "false");
		props.put("mail.smtp.host", "syksmtp.stryker.com");
		...
		try {
			...
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse("chethan.at@stryker.com"));
			message.setRecipients(Message.RecipientType.CC, InternetAddress.parse("harshitha.ul@stryker.com"));
			message.setSubject("Draft email subject");
			message.setText("This is a draft email message.");

			// Set message flags to mark it as a draft
	...
	public static String encrypt(String plainPassword) {
		return java.util.Base64.getEncoder().encodeToString(plainPassword.getBytes(StandardCharsets.UTF_8));
	}
	...
	private static String getTextFromMimeMultipart(MimeMultipart mimeMultipart) throws Exception {
		...
		if (multipartAlternative) {
			// Return only the first body part
			return getTextFromBodyPart((BodyPart) mimeMultipart.getBodyPart(0));
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(decrypt("Q2hldGh1QDI2"));
		//sendMail();
